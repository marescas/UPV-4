...
# Fecha:
# Alumno(s): Marcos Esteve Casademunt y Enric Bonet Cortés
...
def remove_seams(height,seam_paths, matrix):
    """
    COMPLETE eliminar todos los paths que ha calculado en una iteracion
    """
    for y in range(height):
        puntos = sorted([p[y] for p in seam_paths], key=lambda i:-i)
        for punto in puntos:
            matrix[y].pop(punto)
...
def dp_seam_carving_multi(grad,mat,N,pscore=0.7):
    ...
    paths = []
    s = set({})
    while len(paths) < N: #Numero máximo de paths a eliminar en una iteración
        minimo = [infty,0]
        coordenadas = (0,0)
        for x in range(1,width-1): #minimo de la ultima fila
            if (height-1,x) not in s: #el pixel que estamos visitando no lo hemos visitado ya
                if minimo[0] > mat[height-1][x]:
                    minimo = [mat[height-1][x],x]
                    coordenadas = (height-1,x)
        s.add(coordenadas)
        if paths == []: #primera iteración
            best_score = minimo[0] #La mejor puntuación  será la del primer camino calculado
        score = minimo[0]
        if score*pscore <= best_score: #si mi puntuación es menor que mi best_score
            path = [minimo[1]] #nuevo path
            aux = set({})
            salto = False
            for y in range(height-2,-1,-1): #desde la penultima fila
                mv,mpos = min((mat[y][i], i) for i in range(path[-1]-1,path[-1]+2)) #calculamos el seam
                if (y,mpos) in s: #si esta en s --> intersección ya hemos visitado ese pixel
                    salto = True
                    break #Salimos del for y descartamos ese path
                aux.add((y,mpos))
                path.append(mpos)
            if salto: #Pasamos a la siguiente iteración del while en caso de existir un pixel ya visitado
                continue
            for t in aux: #añadimos a visitados  todos los pixeles de ese path
                s.add(t)
            path.reverse()
            paths.append(path) #añadimos el path
        else: #si superamos el bscore salimos
            break
    # COMPLETE HERE
    return paths
