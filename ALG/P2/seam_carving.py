...
def incremental_compute_gradient(grad,img,path):
    ...
    for y in (0,height-1): #La primera y la ultima fila se calculan de forma distinta
        for x in range(max(path[y]-2,1),min(path[y]+2,width-1)): # first and last columns are excluded
            grad[y][x] = abs(img[y][x-1] - img[y][x+1])
...
def dp_seam_carving(grad,mat):
    ...
    minimo = [infty,0] #Calculamos el minimo, (valor,posición)
    for x in range(1,width-1): #Para la ultima fila buscamos el mínimo
        if minimo[0] > mat[height-1][x]:
            minimo = [mat[height-1][x],x]

    path = [minimo[1]] #añadimos el minimo al path
    for y in range(height-2,-1,-1): #descendemos desde la penultima fila
        mv,mpos = min((mat[y][i], i) for i in range(path[-1]-1,path[-1]+2)) #Buscamos el minimo a partir de la ultima posición del path -1, 0, 1
        path.append(mpos) #añadimos la posición minima al path
    path.reverse()
